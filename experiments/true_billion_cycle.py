...
def run_true_billion():
    print("=== TRUE ONE BILLION CYCLE STRESS TEST (CPU) ===")
    ...
    print("This will take approximately 12-15 minutes per configuration.")
    print("Executing in background... check logs for progress.")
    ...
    for name, use_beta, use_h in configs:
        ...
        for step in range(50 + steps): 
            ...
            if step % 1000 == 0 and step > 0:
                print(f"  [{name}] Progress: {step}/{steps} steps...")
                
        total_trials = batch_size * steps
        rate = (success_count / total_trials) * 100
        elapsed = time.time() - start_time
        print(f"  >> {name:25s} : {success_count:13,d} / {total_trials:,} ({rate:6.4f}%) | Time: {elapsed:.1f}s\n")
