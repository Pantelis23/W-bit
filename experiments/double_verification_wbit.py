...
def double_verification():
    print("=== DOUBLE VERIFICATION: Adaptive Logic Extremes ===")
    
    # We will test two extreme scenarios to find the absolute breaking point.
    ...
    def run_scenario(noise_level, description):
        success_count = 0
        ...
        print(f"{description} (Noise {noise_level}): {success_count}/{trials} ({rate:.1f}%)")
        return rate

    print("\n--- Verifying Robustness ---")
    run_scenario(5.0, "Standard High Noise")
    run_scenario(10.0, "Catastrophic Noise (2x Signal)")
    run_scenario(20.0, "Absurd Noise (4x Signal)")
