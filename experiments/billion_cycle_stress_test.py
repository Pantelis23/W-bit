...
def run_billion():
    print("=== ONE BILLION CYCLE STRESS TEST (PyTorch Accelerated) ===")
    noise_level = 5.0
    R_max = 9
    R_eff = 3
    dt = 1.0
    ...
    for name, use_beta, use_h in configs:
        ...
        if use_beta:
            # Translated correctly: Beta adds a constant physical current bias to active states
            drive_bias[allowed_mask] = math.log(bin_size) * beta_multiplier
            
        V_m = torch.zeros(batch_size, R_max)
        success_count = 0
        
        for step in range(50 + steps): # 50 warmup
            ...
            if step >= 50:
                ...
                # target is state 7 (index 2 in the projected array)
                success_count += (winners == 2).sum().item()
                
        total_trials = batch_size * steps
        rate = (success_count / total_trials) * 100
        elapsed = time.time() - start_time
        print(f"  {name:25s} : {success_count:13,d} / {total_trials:,} ({rate:6.3f}%) | Time: {elapsed:.1f}s")
